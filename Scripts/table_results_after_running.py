...
# Create the output table with counts (table_results.txt) after running the pipeline
# Usage: python3 Scripts/table_results_after_running.py "GAGA-0245" "/home/projects/ku_00039/people/igngod/Gene-annotation-pipeline-main/Data/gene_families.xlsx" "/home/projects/ku_00039/people/igngod/Gene-annotation-pipeline-main/Data/Genomes/GAGA-0245"
...
with open(genome_dir+"/table_results.txt", "w") as fp:
    fp.write("Gene/Gene family\tStep_1_complete\tStep_1_partial\tStep_1_chimera\tStep_1_total\tNumber of annotated genes identified\tNumber of putative not annotated genes\tTotal number of identified genes (Annotated + Genomic)\tTotal number of identified genes clustering identical sequences\tTotal number of identified genes clustering highly identical sequences and proteins shorter than 30 aa\tAverage number in ant genomes\tFINAL RESULTS\tTRIMMED\n")
    fp.close()
for i in range(len(gene_families)):
    merged = genome_dir+"/gene_families_pipeline/"+gene_families[i]+"/Step1/"+gene_families[i]+"_merged.txt"
    ...
    result= genome_dir+"/gene_families_pipeline/"+gene_families[i]+"/Step2_bitacora/"+Genome+"_genecounts_summary.txt"
