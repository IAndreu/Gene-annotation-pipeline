...
open (Results, ">", "$ARGV[1]_parsed_blast.txt");
foreach my $key (sort keys %blast) {
	...
	if ($hitlvl == 2){
		...
		if ($hits == 1){
			...
			print Results "$key $ipos $fpos $length{$key} blastp\n";
		}
		else {
			my $n = 0;
			foreach my $i (@ini){
				...
				my $nn= $n+1;
				print Results "$key\_split$nn $ipos $fpos $length{$key} blastp\n";
				$n++;
