...
while (<File>) {
	...
	if (exists $gffgene{$gene}){
		...
		print Results "$subl2[0]\tAnnotGFF\tgene\t$subl2[3]\t$subl2[4]\t$subl2[5]\t$subl2[6]\t$subl2[7]\tID=$geneparent;$subline[4];Pos:$subline[1]-$subline[2]\n";
		print Results "$subl2[0]\tAnnotGFF\tmRNA\t$subl2[3]\t$subl2[4]\t$subl2[5]\t$subl2[6]\t$subl2[7]\tID=$subline[0];Parent=$geneparent;$subline[4];Pos:$subline[1]-$subline[2]\n";
...
###new - ordering CDS
		...
		foreach my $posit (@posorder){
			foreach my $cds (@{$gffcds{$gene}{$posit}}) {
				my @subl3 = split (/\t/, $cds);
				print Results "$subl3[0]\tAnnotGFF\tCDS\t$subl3[3]\t$subl3[4]\t$subl3[5]\t$subl3[6]\t$subl3[7]\tID=$subline[0]\.CDS$n;Parent=$subline[0];$subline[4];Pos:$subline[1]-$subline[2]\n";		
				$n++;
			}
		}
###
...
open (Results, ">", "$ARGV[2]\_annot_genes_trimmed.gff3");
...
while (<File>) {
	...
	if (exists $gffgene{$gene}){
		...
		my $firstcds = $subline[1]; ## Filter to avoid modifying the first cds if the alignment starts from 1 position
		my $ncds = 1;
		...
		foreach my $cds (@{$gffcds{$gene}{$posit}}) {
			...
			if ($inicuted == 0){
				$inicuted = $subl3[3] + ($subline[1]*3) - $precdslength - $nifix + $subl3[7] -3; # $subl3[7] para el frame que codificaría coincida
			} else {
				$inicuted = $subl3[3];
			}

			if ($endcuted == 0){
				$endcuted = $subl3[3] + ($subline[2]*3) - $prelength - $precdslength - $nifix +3;
			} elsif ($endgenereached == 0){
				my $diflength = $subline[2] - $subline[1]; 
			...
			if ($inicuted >= $subl3[3] && $endcuted <= $subl3[4]){
				...
				$printcds .= "$subl3[0]\tAnnotGFF\tCDS\t$inicuted\t$endcuted\t$subl3[5]\t$subl3[6]\t$subl3[7]\tID=$cdsid;Parent=$subline[0];$subline[4];Pos:$subline[1]-$subline[2]\n";
				...
				if ($firstcds == 1){ ## Change to print unmodifyied CDS if alignment starts in 1
					$printcds .= "$subl3[0]\tAnnotGFF\tCDS\t$subl3[3]\t$subl3[4]\t$subl3[5]\t$subl3[6]\t$subl3[7]\tID=$cdsid;Parent=$subline[0];$subline[4];Pos:$subline[1]-$subline[2]\n";
					$firstcds++;
				} else {
					$printcds .= "$subl3[0]\tAnnotGFF\tCDS\t$inicuted\t$subl3[4]\t$subl3[5]\t$subl3[6]\t$subl3[7]\tID=$cdsid;Parent=$subline[0];$subline[4];Pos:$subline[1]-$subline[2]\n";
				}
		...
		foreach my $cds (@{$gffcds{$gene}{$posit}}) {
			...
			if ($inicuted == 0){
				$inicuted = $subl3[4] - ($subline[1]*3) + $precdslength + $nifix - $subl3[7] +6; # $subl3[7] para el frame que codificaría coincida
			} else {
				$inicuted = $subl3[4];
			}

			if ($endcuted == 0){
				$endcuted = $subl3[4] - ($subline[2]*3) + $prelength + $precdslength + $nifix;
			} elsif ($endgenereached == 0){
				my $diflength = $subline[2] - $subline[1]; 
			...
			if ($inicuted <= $subl3[4] && $endcuted >= $subl3[3]){
				$endgenereached++;
				next if ($endcuted >= $inicuted); ## check gene structure
				$printcds .= "$subl3[0]\tAnnotGFF\tCDS\t$endcuted\t$inicuted\t$subl3[5]\t$subl3[6]\t$subl3[7]\tID=$cdsid;Parent=$subline[0];$subline[4];Pos:$subline[1]-$subline[2]\n";
				#$printcds .= "$subl3[0]\tAnnotGFF\tCDS\t$subl3[3]\t$subl3[4]\t$subl3[5]\t$subl3[6]\t0\tID=$cdsid;Parent=$subline[0];$subline[4];$subline[1];Pos:$subline[2]-$subline[3]\n";
				#if ($inigene == 0 || $inigene )
			} 
				...
				if ($firstcds == 1){ #### Change to print unmodifyied CDS if alignment starts in 1
					...
					$printcds .= "$subl3[0]\tAnnotGFF\tCDS\t$subl3[3]\t$inicuted\t$subl3[5]\t$subl3[6]\t$subl3[7]\tID=$cdsid;Parent=$subline[0];$subline[4];Pos:$subline[1]-$subline[2]\n";
				}
		...
		print Results "$subl2[0]\tAnnotGFF\tgene\t$inigene\t$endgene\t$subl2[5]\t$subl2[6]\t$subl2[7]\tID=$geneparent;$subline[4];Pos:$subline[1]-$subline[2]\n";
		print Results "$subl2[0]\tAnnotGFF\tmRNA\t$inigene\t$endgene\t$subl2[5]\t$subl2[6]\t$subl2[7]\tID=$subline[0];Parent=$geneparent;$subline[4];Pos:$subline[1]-$subline[2]\n";
